...
class Flight03 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  class ComplexNum(var real:Double, val imaginary: Double){
    override def toString = if(imaginary < 0) s"${real} - ${abs(imaginary)}i" 
          else  s"${real} + ${imaginary}i"
    def +(that: ComplexNum): ComplexNum = 
                new ComplexNum(this.real + that.real, this.imaginary + that.imaginary)
    def +(d: Double): ComplexNum = new ComplexNum(this.real + d, this.imaginary)
    def this(r: Double) = this(r, 0)
  }

  implicit def doubleToComplexNum(d: Double) = new ComplexNum(d)
